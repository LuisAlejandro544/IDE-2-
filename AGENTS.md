...
4. **Sistema de Skills Integradas Detrás de Escena (`/skills/*.md`)**: Carga automática e inyección en prompt del Agente de IA:
   - 🛠️ `tool_usage_guide.md`: Guía de herramientas, prohibición estricta de reescribir archivos enteros con `edit_file`.
   - 🎨 `design_ui_ux.md`: Diseños M3, paleta de colores y estética moderna.
   - 📱 `responsive_layout.md`: Adaptabilidad responsive Mobile-First y Flexbox/Grid.
   - ⚡ `clean_logic.md`: Lógica robusta, ES6+, Import Maps, ES Modules, código limpio y manejo de errores.
   - ⚡ `cpp_development.md`: Desarrollo C++ modular (C++ Web vía WebAssembly y C++ App nativa NDK/JNI).
