    ...
    suspend fun ensureDefaultProjectsExist() {
        if (projectDao.getProjectCount() == 0) {
            ...
            val defaultProject4 = ProjectEntity(
                name = "C++ Web Application",
                description = "Aplicación Web compilada desde C++ (WebAssembly / Emscripten) ejecutable en navegador.",
                framework = "C++ Web",
                iconEmoji = "🌐"
            )
            val p4Id = projectDao.insertProject(defaultProject4)
            templateDelegate.createCppWebTemplate(p4Id)

            val defaultProject5 = ProjectEntity(
                name = "C++ Native Android App",
                description = "Aplicación móvil nativa en C++ con bridge JNI y NDK.",
                framework = "C++ App",
                iconEmoji = "📱"
            )
            val p5Id = projectDao.insertProject(defaultProject5)
            templateDelegate.createCppAppTemplate(p5Id)

            val defaultProject6 = ProjectEntity(
                name = "React 18 App",
                description = "Aplicación Web interactiva en React 18 + Import Maps y Módulos ES.",
                framework = "React Puro",
                iconEmoji = "⚛️"
            )
            val p6Id = projectDao.insertProject(defaultProject6)
            templateDelegate.createReactPureTemplate(p6Id)
        }
    }
